...
Vector3 Raytrace::computeLocalColor(const Intersection &intersection) {
  ...
  */
  ...
  float intensiteDiffusee = 0;
  float intensiteSpecular = 0;
  Vector3 R;
  ...
  for(int i = 0; i < _scene->nbLight(); i++){
      ...
      Ray shadow(P,L);
      Intersection *plusProche = _scene->intersection(shadow,1.0);
      if(plusProche != NULL){
          if(plusProche->lambda()>1){
            intensiteDiffusee = max(dot(N,L), 0.0);
            intensiteSpecular = pow(max(dot(V,R),0.0),m.shininess());
          }
      }
      else{
        intensiteDiffusee = max(dot(N,L), 0.0);
      ...
      result+= intensiteDiffusee * m.diffuse() + intensiteSpecular * m.specular();
      delete(plusProche);
  }
