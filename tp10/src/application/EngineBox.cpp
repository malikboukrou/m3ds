...
void EngineBox::computeForce() {
  for(unsigned int i=0; i<_boxList->size(); i++) {
    ...
    //ex2 q1
    double G = 9.81;
    Vector3 v = Vector3(0,-5*b->mass()*G,0);
    b->addForce(v);

    //ex3 q4
    double frottement = 0.8;
    b->addForce(-1.5*frottement*b->velocity());
    b->addMoment(-1.5*frottement*b->omega());
    ...
    if(_cursorActive){
          ...
          Vector3 force = 15*(_cursor - b->attachWorld());
          b->addForce(force);
          //ex3 q3
          b->addMoment(force.cross(b->position()-b->attachWorld()));
      }
  }
